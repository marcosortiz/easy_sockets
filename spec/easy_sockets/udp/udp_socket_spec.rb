...
describe EasySockets::UdpSocket do
    ...
    let :opts do
        {
            :host => host,
            :port => port,
            :logger => Logger.new("#{TEST_LOGS_PATH}/udp_client.log")
        }
    end
    
    def udp_socket(options={})
        MyTestUdpSocket.new(opts.merge(options))
    end
    ...
    describe 'connecting' do
        ...
        context 'with server not available' do
            it 'must NOT raise any exception' do
                s = udp_socket( port: 9999 )
                10.times do
                    s.connect
                    check_connected(s, true)
                    expect(s.connect_count).to eq 1
    ...
    describe 'send_msg' do
        ...
        context 'separators' do
            ...
            let(:my_socket) do
                Class.new(EasySockets::UdpSocket) do
        ...
        context 'when msg size is smaller than CHUNK_SIZE' do
            ...
            let :range do
                (0..EasySockets::CHUNK_SIZE/10)
            end
            
            let :step do
                EasySockets::CHUNK_SIZE / 100
            end
