...
require 'easy_sockets/udp/udp_server'
...
class MyTestUdpSocket < EasySockets::UdpSocket
    attr_reader :connect_count, :disconnect_count
...
def start_udp_server(port=2500)
    @servers ||= []
    t = Thread.new do
        opts = {
            logger: Logger.new("#{TEST_LOGS_PATH}/udp_server.log"),
            port: port,
        }
        server = EasySockets::UdpServer.new(opts)
        server.start
    end
    @servers << t
    t.join(0.01)
